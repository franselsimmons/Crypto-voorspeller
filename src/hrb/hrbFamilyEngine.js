import { hrbCfg, HRB_FAMILY_IDS } from "./config.js";
import { rcmd, rpipe } from "../storage/redis.js";
import { HK } from "./hrbKeys.js";
import { acquireLock, releaseLock } from "../security/locks.js";
import { bootstrapLcb, parentProbsFor, OUTCOME_CATS, outcomeValues } from "../verification/statistics.js";
import { benjaminiHochberg } from "../verification/fdr.js";

/**
 * HRB-eigen familie-engine. Structureel identiek aan ARS-U's v3 (F1/F11-veilig:
 * geserialiseerd onder eigen lock, bumpSeen accepteert een aantal), maar werkt op
 * de HRB-namespace en 4 families (richting × klasse). Géén Discord-posts in de
 * leerfase — HRB meet stil. parentProbs aggregeert per richting over HRB-families.
 */
function emptyFamily(fid) {
  return {
    familyId: fid, seen: 0, open: 0, completed: 0,
    counts: { loss: 0, be: 0, full: 0, timeout: 0 },
    wins: 0, losses: 0, grossR: 0, netR: 0, cum: 0, peak: 0,
    maxDrawdownR: 0, currentDrawdownR: 0, bestStreak: 0, worstStreak: 0, streak: 0, lastR: [],
    status: "COLLECTING", verifiedAt: null, lostEdgeAt: null,
    avgGrossR: null, avgNetR: null, medianR: null, profitFactor: null,
    winrate: null, last30AvgR: null, last50AvgR: null,
    lcb: null, pValue: null, fdrPass: false,
  };
}

const famKey = (fid) => HK.family(hrbCfg().namespace, fid);

async function withLock(fn) {
  const token = await acquireLock("hrb:families", 8000, 40, 100);
  if (!token) throw new Error("hrb-families-lock niet verkregen");
  try { return await fn(); } finally { await releaseLock("hrb:families", token); }
}

export async function loadHrbFamilies() {
  const raw = await rpipe(HRB_FAMILY_IDS.map((fid) => ["GET", famKey(fid)]));
  const out = {};
  HRB_FAMILY_IDS.forEach((fid, i) => { out[fid] = raw[i] ? JSON.parse(raw[i]) : emptyFamily(fid); });
  return out;
}

export async function hrbBumpSeen(fid, count = 1) {
  if (!Number.isInteger(count) || count < 1) return null;
  return withLock(async () => {
    const f = JSON.parse((await rcmd("GET", famKey(fid))) || "null") || emptyFamily(fid);
    f.seen += count; f.open += count;
    await rcmd("SET", famKey(fid), JSON.stringify(f));
    return f;
  });
}

export async function hrbRecordClose(fid, outcome) {
  return withLock(async () => {
    const f = JSON.parse((await rcmd("GET", famKey(fid))) || "null") || emptyFamily(fid);
    f.open = Math.max(0, f.open - 1);
    f.completed++;
    f.counts[outcome.category] = (f.counts[outcome.category] || 0) + 1;
    f.grossR += outcome.grossR; f.netR += outcome.netR; f.cum += outcome.netR;
...
export async function recomputeHrbFamilies() {
  const c = hrbCfg();
  ...
  return withLock(async () => {
    ...
    for (const fid of HRB_FAMILY_IDS) {
      const f = fams[fid];
      const nn = f.completed;
      if (nn > 0) {
        f.avgGrossR = Number((f.grossR / nn).toFixed(4));
        f.avgNetR = Number((f.netR / nn).toFixed(4));
        ...
        f.winrate = Number(((f.wins / nn) * 100).toFixed(2));
        const lastN = (k) => { const arr = f.lastR.slice(-k); return arr.length ? Number((arr.reduce((s, v) => s + v, 0) / arr.length).toFixed(4)) : null; };
        f.last30AvgR = lastN(30); f.last50AvgR = lastN(50);
    ...
    const passSet = benjaminiHochberg(
      HRB_FAMILY_IDS.map((fid) => ({ id: fid, p: fams[fid].completed >= c.minTotalPerFamily ? fams[fid].pValue : null })),
      c.bhAlpha
    );
    const changes = [];
    for (const fid of HRB_FAMILY_IDS) {
    ...
    const cmds = HRB_FAMILY_IDS.map((fid) => ["SET", famKey(fid), JSON.stringify(fams[fid])]);
    for (const ch of changes) cmds.push(["LPUSH", HK.familyStatusLog(), JSON.stringify(ch)]);
    cmds.push(["LTRIM", HK.familyStatusLog(), 0, 199]);
    await rpipe(cmds);
