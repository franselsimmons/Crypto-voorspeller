...
import { getDashboardData, parseDashboardFilters, type SearchParams } from "@/lib/dashboard";
...
export default async function Home({
  ...
  const totalRTone = data.overview.totalR > 0 ? "good" : data.overview.totalR < 0 ? "bad" : "default";
  const pnlTone = data.overview.pnlPct > 0 ? "good" : data.overview.pnlPct < 0 ? "bad" : "default";
  ...
  return (
    <main className="shell">
      ...
      <section className="metric-grid">
        ...
        <MetricCard
          label="Profit factor"
          value={data.overview.profitFactor === null ? "—" : compactNumber(data.overview.profitFactor, 2)}
