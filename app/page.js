...
export default function Home() {
    ...
    return (
        <main className="min-h-screen bg-[#030303] text-zinc-400 p-4 md:p-16 font-mono uppercase tracking-tighter">
            <div className="max-w-4xl mx-auto">
                ...
                <header className="flex flex-col md:flex-row justify-between items-start md:items-baseline mb-12 border-b border-zinc-900 pb-8 gap-6">
                    ...
                    <button 
                        onClick={scanMarket} 
                        disabled={loading} 
                        className="w-full md:w-auto border border-zinc-800 hover:border-zinc-200 hover:text-white px-8 py-3 text-[10px] transition-all disabled:opacity-20"
                    >
                ...
                {allCoins.length > 0 ? (
                    <div className="space-y-4">
                        {allCoins.map((c, i) => (
                            <div key={i} className={`relative bg-zinc-950/40 border border-zinc-900 p-6 transition-all duration-500 hover:bg-zinc-900/40 ${i === 0 ? 'border-l-2 border-l-emerald-500' : ''}`}>
                                ...
                                <div className="flex justify-between items-start mb-6">
                                    ...
                                    <button 
                                        ...
                                        className="text-[9px] text-zinc-500 border border-zinc-800 px-3 py-1.5 hover:text-white hover:border-zinc-500"
                                    >
                                        COPY_DATA
                                    </button>
                                </div>
                                ...
                                <div className="mt-5 flex gap-2">
                                    {c.tags.slice(0, 2).map((t, x) => (
                                        <span key={x} className="text-[8px] text-zinc-700 bg-zinc-900/50 px-2 py-0.5 rounded-sm">
                                            // {t}
                                        </span>
                                    ))}
                                </div>
                ...
                <footer className="mt-20 pt-8 border-t border-zinc-900 flex flex-col md:flex-row justify-between gap-4 text-[8px] text-zinc-800 tracking-[0.2em]">
                    <span>FUTURES_ONLY_TERMINAL // V2.2</span>
                    <span>&copy; 2026 ACCESS_RESTRICTED</span>
                </footer>
            </div>
        </main>
    );
