...
export function calculateCryptoCroc(klines, btcTrend = 'neutral') {
    ...
    // BTC Alignment (Zwaarste weging)
    if (btcTrend !== 'neutral') {
        if (type === btcTrend) { score += 30; reasonTags.push("BTC_CONFLUENCE"); }
        else { score -= 25; reasonTags.push("ANTI_MARKET"); }
    }

    // Trend alignment
    if ((type === "long" && emaDistancePct > 0) || (type === "short" && emaDistancePct < 0)) {
        score += 15; reasonTags.push("TREND_ALIGNED");
    }
    ...
    if (hasHighVolume) { score += 10; reasonTags.push("VOL_SPIKE"); }
