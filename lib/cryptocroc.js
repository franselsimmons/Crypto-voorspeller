...
// Hulpfunctie voor mooie weergave van de prijzen
...
export function calculateCryptoCroc(klines) {
    ...
    // 2. Trend Filter (50 EMA voor de 1H grafiek)
    ...
    // 4. Divergentie Check (Kijkt naar de bodems en toppen van de RSI vs Prijs)
    ...
    // 5. CryptoCroc Banden (Stress, Compressie en Dynamische Zones)
    ...
    // 6. Signaal Genereren & Labels Plakken
    let signal = "NEUTRAAL";
    let type = "none";
    let tags = [];
    let multiplier = 1.0;
    ...
    // 7. STRUCTURAL TP & SL Berekening (Gebaseerd op Marktstructuur)
    let sl = 0;
    let tp1 = 0; 
    let tp2 = 0; 
    
    // Kijk 24 uur terug voor de échte swing highs en swing lows
    const lookback = 24;
    ...
    if (type === 'long') {
        // Riskeer tot net onder de recente bodem
        const structuralRisk = currentPrice - swingLow;
        // Failsafe: Nooit een belachelijk kleine SL, gebruik dan minimaal 1.5x ATR
        const actualRisk = Math.max(structuralRisk, currentAtr * 1.5); 
        
        sl = currentPrice - actualRisk;
        tp1 = currentPrice + (actualRisk * 1.0); // 1:1 Risk/Reward
        tp2 = currentPrice + (actualRisk * 2.0); // 1:2 Risk/Reward
        
    } else if (type === 'short') {
        // Riskeer tot net boven de recente top
