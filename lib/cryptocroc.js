...
export function calculateCryptoCroc(klines, btcTrend = 'neutral') {
    ...
    // --- DE "AGREEMENT" SCORING ---
    let score = 0;
    let reasonTags = [];

    // Basis: RSI afstand (Dictator van de score)
    const rsiDist = Math.abs(currentRsi - 50);
    score += (rsiDist * 1.2); 

    // De "Eens zijn" Check (De Filter)
    const agreement = (btcTrend === 'long' && type === 'long') || (btcTrend === 'short' && type === 'short');

    if (agreement) {
        score += 40; // Mega bonus als ze het eens zijn
        reasonTags.push("AGREEMENT_CONFIRMED");
    } else {
        score -= 40; // Zware straf bij onenigheid
