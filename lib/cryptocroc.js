...
export function calculateCryptoCroc(klines) {
    ...
    // 2. GRADUELE TREND SCORE (50 EMA)
    const ema50 = EMA.calculate({ period: 50, values: closes });
    const currentEma50 = ema50.length > 0 ? ema50[ema50.length - 1] : currentPrice;
    
    // Hoe ver zit de prijs van de EMA vandaan in procenten?
    ...
    // 6. ACTIONABILITY & TYPE BEPALEN
    let actionability = "WATCHLIST"; // Default fallback
    ...
    // 7. RANKING SCORE BEREKENEN (Wordt ALTIJD berekend, ook bij neutraal)
    // Basis = pure RSI stretch (0 tot 100)
    let baseScore = Math.abs(currentRsi - 50) * 2; 
    let scoreMultiplier = 1.0;

    if (type === 'long') {
        scoreMultiplier += trendScore; // Kan positief of negatief zijn!
        if (hasHighVolume) { tags.push("Volume+"); scoreMultiplier += 0.3; } 
        if (bullishDiv) { tags.push("Bull Div"); scoreMultiplier += 0.8; } 
    } else {
        scoreMultiplier -= trendScore; // Bij short is een negatieve trendScore goed (- - = +)
        if (hasHighVolume) { tags.push("Volume+"); scoreMultiplier += 0.3; }
        if (bearishDiv) { tags.push("Bear Div"); scoreMultiplier += 0.8; }
    }

    // Zorg dat de score binnen logische grenzen blijft
