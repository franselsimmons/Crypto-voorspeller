...
export function calculateCryptoCroc(klines, btcTrend = 'neutral') {
    ...
    // Type bepalen op basis van RSI stand
    let type = currentRsi < 50 ? "long" : "short";
    
    // ==============================================================
    // --- PULLBACK SCORING MATRIX (Buy the Dip / Sell the Rip) ---
    // ==============================================================
    let score = 0; 
    let reasonTags = [];

    // 1. RSI Stretch (Basis: 1.5x de afstand tot 50)
    const rsiDist = Math.abs(currentRsi - 50);
    score += (rsiDist * 1.5);

    // 2. De Macro Pullback Logica (Exact wat jij vroeg)
    if (btcTrend === 'long') {
        if (type === 'long') {
            // BTC is hoog, RSI is laag = PERFECTE DIP KANS
            score += 30; 
            reasonTags.push("DIP_IN_UPTREND");
        } else {
            // BTC is hoog, RSI is hoog = TE LAAT OM TE KOPEN
            score -= 20; 
            reasonTags.push("OVERBOUGHT");
        }
    } else if (btcTrend === 'short') {
        if (type === 'short') {
            // BTC is laag, RSI is hoog = PERFECTE SHORT KANS
            score += 30; 
            reasonTags.push("RIP_IN_DOWNTREND");
        } else {
            // BTC is laag, RSI is laag = VANG GEEN VALLENDE MESSEN
            score -= 20; 
            reasonTags.push("OVERSOLD_IN_BEAR");
        }
    }
    ...
    // Finaliseer de score (Limiet tussen 0 en 99)
    const finalScore = Math.min(99, Math.max(0, score));
    
    // Drempel iets verlaagd naar 45 zodat je sneller kansen ziet in normale markten
    const isActionable = finalScore >= 45;

    // ==============================================================

    // Structural SL/TP (24h Lookback)
    const lookback = 24;
