...
// Hulpfunctie: Coins hebben hele rare prijzen (BTC is 70000, PEPE is 0.000007). Dit formatteert het mooi.
...
export function calculateCryptoCroc(klines) {
    ...
    // NIEUW: EXTREEM PRECIEZE TP EN SL BEREKENING (ATR Methode)
    let sl = 0;
    let tp = 0;
    
    // Stop Loss = 1.5x de ATR. Take Profit = 3.0x de ATR. (Risk:Reward = 1:2)
    if (type === 'long') {
        sl = currentPrice - (currentAtr * 1.5);
        tp = currentPrice + (currentAtr * 3.0);
    } else if (type === 'short') {
        sl = currentPrice + (currentAtr * 1.5);
        tp = currentPrice - (currentAtr * 3.0);
    }
    ...
    return {
        ...
        // Geef de strakke geformatteerde prijzen terug naar de website
